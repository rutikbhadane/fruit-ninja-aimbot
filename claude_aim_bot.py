...
# ══════════════════════════════════════════════════════════════════
#  STEP 1 — Find BlueStacks window + compute regions automatically
# ══════════════════════════════════════════════════════════════════
...
print(f"BlueStacks client: {CLIENT_W}x{CLIENT_H} at ({CLIENT_ORIGIN_X}, {CLIENT_ORIGIN_Y})")
...
def _focus_bluestacks():
    """Bring BlueStacks to foreground so it receives input."""
    _SetForeground(HWND)
...
def swipe(x1, y1, x2, y2):
    ...
    Uses SetCursorPos + mouse_event — confirmed working on BlueStacks 5.
    """
...
class FruitNinjaBot:
    ...
    def run(self):
        print("\n" + "="*50)
        print("Bot starting — focusing BlueStacks in 2s...")
        print("="*50)
        time.sleep(2)
        _focus_bluestacks()
        ...
        while not self._stop.is_set():
            ...
            if safe:
                # Re-focus BlueStacks every 60 frames
