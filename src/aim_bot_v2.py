...
def find_fruit_ninja_window():
    ...
    'Fruit Ninja' or 'FruitNinja' (case-insensitive). This handles
    variations like 'Fruit Ninja®', 'Fruit Ninja Classic', etc.
    Returns the HWND or 0 if not found.
    """
    found = [0]

    EnumWindowsProc = ctypes.WINFUNCTYPE(
        ctypes.c_bool,
        ctypes.c_void_p,   # HWND as plain int
...
def get_window_rect():
    """
    Locate the Fruit Ninja window (any title variant) and return its
    geometry as (hwnd, screen_x, screen_y, client_width, client_height).
    Returns (None, None, None, None, None) if not found.
    """
    # 1. Fast path — try known exact titles first
    hwnd = 0
    ...
    # 2. Slow path — enumerate all visible windows
    if not hwnd:
